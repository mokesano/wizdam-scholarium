...
class CitationDAO extends DAO {
	...
	function importCitations(&$request, $assocType, $assocId, $rawCitationList) {
		...
		import('core.classes.citation.CitationListTokenizerFilter');
	...
	function &_filterCitation(&$request, &$citation, &$filterCallback, $citationStateAfterFiltering, $fromFilterIds = array()) {
		...
		if (empty($filterList)) {
			...
			import('core.classes.filter.GenericMultiplexerFilter');
			...
			// meta-data schemas.
			import('core.plugins.metadata.nlm30.filter.Nlm30CitationDemultiplexerFilter');
			...
			import('core.classes.filter.GenericSequencerFilter');
