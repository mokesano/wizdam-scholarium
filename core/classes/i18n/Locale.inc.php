...
import('core.classes.i18n.LocaleFile');
...
class Locale {
	...
	function translate($key, $params = array(), $locale = null) {
		...
		if (!HookRegistry::call('Locale::translate', array(&$this, &$key, &$params, &$locale, &$localeFiles, &$value))) {
	...
	function &registerLocaleFile ($locale, $filename, $addToTop = false) {
		...
		if (!HookRegistry::call('Locale::registerLocaleFile::isValidLocaleFile', array(&$localeFile))) {
		...
		HookRegistry::call('Locale::registerLocaleFile', array(&$locale, &$filename, &$addToTop));
		return $localeFile;
	}
	...
	function installLocale($locale) {
		...
		import('core.classes.db.DBDataXMLParser');
		...
		HookRegistry::call('Locale::installLocale', array(&$locale));
	}
...
/**
 * Wrapper around Locale::translate().
 *
