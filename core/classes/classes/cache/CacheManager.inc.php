...
import('core.classes.cache.FileCache');
...
class CacheManager {
	...
	function &getCache($context, $cacheId, $fallback, $type = CACHE_TYPE_FILE) {
		switch ($this->getCacheImplementation($type)) {
			case 'xcache':
				import('core.classes.cache.XCacheCache');
				$cache = new XCacheCache(
					$context, $cacheId, $fallback
				);
				break;
			case 'apc':
				import('core.classes.cache.APCCache');
				$cache = new APCCache(
					$context, $cacheId, $fallback
				);
				break;
			case 'memcache':
				import('core.classes.cache.MemcacheCache');
			...
			case 'file':
				...
				break;
			case 'none':
				import('core.classes.cache.GenericCache');
