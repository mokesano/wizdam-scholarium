...
class FileWrapper {
	...
	function &wrapper($source) {
		if (ini_get('allow_url_fopen') && Config::getVar('general', 'allow_url_fopen') && is_string($source)) {
			...
			import('core.classes.file.wrappers.ResourceWrapper');
			...
			switch ($scheme) {
				case 'http':
					import('core.classes.file.wrappers.HTTPFileWrapper');
					$wrapper = new HTTPFileWrapper($source, $info);
					$wrapper->addHeader('User-Agent', $userAgent);
					break;
				case 'https':
					import('core.classes.file.wrappers.HTTPSFileWrapper');
					$wrapper = new HTTPSFileWrapper($source, $info);
					$wrapper->addHeader('User-Agent', $userAgent);
					break;
				case 'ftp':
					import('core.classes.file.wrappers.FTPFileWrapper');
