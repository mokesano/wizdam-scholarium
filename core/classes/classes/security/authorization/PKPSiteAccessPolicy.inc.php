...
import('core.classes.security.authorization.PolicySet');

class SiteAccessPolicy extends PolicySet {
	/**
	 * Constructor
	 *
	...
	function PKPSiteAccessPolicy(&$request, $operations, $roleAssignments, $message = 'user.authorization.loginRequired') {
		...
		if(is_array($roleAssignments)) {
			...
			import('core.classes.security.authorization.PKPPublicAccessPolicy');
			$siteRolePolicy->addPolicy(new PublicAccessPolicy($request, $operations));
		} else {
