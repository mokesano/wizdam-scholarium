...
import('core.classes.controllers.grid.GridHandler');
import('core.classes.controllers.grid.citation.PKPCitationGridCellProvider');

// import citation grid specific classes
import('core.classes.controllers.grid.citation.PKPCitationGridRow');
...
class CitationGridHandler extends GridHandler {
	 ...
	 *
	 * @see CitationGridHandler::setAssocObject() for more details.
	 *
	 * @return DataObject
	 */
	...
	function initialize(&$request, $args) {
		...
		// Columns
		$cellProvider = new CitationGridCellProvider();
		$this->addColumn(
			new GridColumn(
				'rawCitation',
				null,
	...
	function editCitation(&$args, &$request) {
		...
		// Form handling
		import('core.classes.controllers.grid.citation.form.CitationForm');
	...
	function &getCitationFromArgs($request, &$args, $createIfMissing = false) {
		...
		if (isset($args['citationId'])) {
			...
			if ($createIfMissing) {
				...
				import('core.classes.citation.Citation');
