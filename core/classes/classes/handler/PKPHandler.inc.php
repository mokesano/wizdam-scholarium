...
import('core.classes.handler.validation.HandlerValidator');
import('core.classes.handler.validation.HandlerValidatorRoles');
import('core.classes.handler.validation.HandlerValidatorCustom');
...
class Handler {
	 ...
	 * call so that Handler::authorize() will be able to enforce
	 * them.
	 *
	...
	function addPolicy($authorizationPolicy, $addToTop = false) {
		if (is_null($this->_authorizationDecisionManager)) {
			...
			import('core.classes.security.authorization.AuthorizationDecisionManager');
	...
	function authorize(&$request, &$args, $roleAssignments) {
		...
		import('core.classes.security.authorization.RestrictedSiteAccessPolicy');
		$this->addPolicy(new RestrictedSiteAccessPolicy($request), true);

		// Enforce SSL site-wide.
		if ($this->requireSSL()) {
			import('core.classes.security.authorization.HttpsPolicy');
		...
		if (!defined('SESSION_DISABLE_INIT')) {
			...
			if (is_a($user, 'User')) {
				import('core.classes.security.authorization.UserRolesRequiredPolicy');
	...
	function &getRangeInfo($rangeName, $contextData = null) {
		...
		if (empty($pageNum)) {
			...
			if ($session && $contextData !== null) {
				...
				$contextHash = Handler::hashPageContext($contextData);
		...
		import('core.classes.db.DBResultRange');
