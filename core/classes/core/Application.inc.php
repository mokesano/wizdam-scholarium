...
class Application {
	...
	function __construct() {
		...
		import('core.classes.core.PKPProfiler');
		$pkpProfiler = new Profiler();

		// Begin debug logging
		Console::logMemory('', 'Application::construct');
		Console::logSpeed('Application::construct');

		// Seed random number generator
		mt_srand(((double) microtime()) * 1000000);

		import('core.classes.core.Core');
		import('core.classes.core.PKPString');
		import('core.classes.core.Registry');

		import('core.classes.config.Config');
		...
		import('core.classes.db.DAORegistry');
		import('core.classes.db.XMLDAO');

		import('core.classes.cache.CacheManager');

		import('app.classes.security.Validation');
		import('core.classes.session.SessionManager');
		import('app.classes.template.TemplateManager');
		import('app.classes.notification.NotificationManager');

		import('core.classes.plugins.PluginRegistry');
		import('core.classes.plugins.HookRegistry');

		import('app.classes.i18n.AppLocale');
	...
	function &getRequest() {
		...
		if (is_null($request)) {
			import('app.classes.core.Request');
	...
	function &getDispatcher() {
		...
		if (is_null($dispatcher)) {
			import('core.classes.core.Dispatcher');
	...
	static function defineExposedConstant($name, $value) {
		...
		$constants =& Application::getExposedConstants(); // KEEP reference assignment for static access
...
function define_exposed($name, $value) {
	...
	if (defined('E_DEPRECATED')) $errorReportingLevel &= ~E_DEPRECATED;
	@error_reporting($errorReportingLevel);
	Application::defineExposedConstant($name, $value); // FIXED: Sekarang bisa dipanggil static
