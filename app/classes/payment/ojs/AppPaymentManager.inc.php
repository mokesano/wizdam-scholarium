...
/**
 * @file classes/payment/ojs/AppPaymentManager.inc.php
 *
...
import('app.classes.payment.ojs.AppQueuedPayment');
import('core.classes.payment.PaymentManager');
...
class AppPaymentManager extends PaymentManager {
	/**
	 * Constructor
	 * @param $request PKPRequest
	 */
	function AppPaymentManager(&$request) {
	...
	function &createQueuedPayment($journalId, $type, $userId, $assocId, $amount, $currencyCode = null) {
		...
		$payment = new AppQueuedPayment($amount, $currencyCode, $userId, $assocId);
	...
	function &createCompletedPayment($queuedPayment, $payMethod) {
		import('app.classes.payment.ojs.AppCompletedPayment');
		...
		if ($queuedPayment) switch ($queuedPayment->getType()) {
			...
			case PAYMENT_TYPE_PURCHASE_SUBSCRIPTION:
				...
				if ($institutional) {
					...
					import('app.classes.subscription.InstitutionalSubscription');
					...
					if ($journalSettingsDao->getSetting($subscription->getJournalId(), 'enableSubscriptionOnlinePaymentNotificationPurchaseInstitutional')) {
						import('app.classes.subscription.SubscriptionAction');
						SubscriptionAction::sendOnlinePaymentNotificationEmail($subscription, 'SUBSCRIPTION_PURCHASE_INSTL');
					}
				} else {
					import('app.classes.subscription.IndividualSubscription');
			...
			case PAYMENT_TYPE_GIFT:
				...
				import('app.classes.mail.MailTemplate');
