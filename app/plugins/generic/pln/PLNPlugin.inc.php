...
import('app.classes.article.PublishedArticle');
import('app.classes.issue.Issue');
...
class PLNPlugin extends GenericPlugin {
	...
	function register($category, $path) {
		...
		if ($success) {
			...
			if ($this->getEnabled()) {
				...
				$this->registerDAOs();
				$this->import('app.classes.Deposit');
				$this->import('app.classes.DepositObject');
				$this->import('app.classes.DepositPackage');
	...
	function registerDAOs() {
		...
		$this->import('app.classes.DepositDAO');
		$this->import('app.classes.DepositObjectDAO');
	...
	function manage($verb, $args, &$message, &$messageParams, $request = null) {
		...
		switch($verb) {
			...
			case 'settings':
				$templateMgr =& TemplateManager::getManager();
				$templateMgr->register_function('plugin_url', array(&$this, 'smartyPluginUrl'));
				$this->import('app.classes.form.PLNSettingsForm');
			...
			case 'status':
				$templateMgr =& TemplateManager::getManager();
				$templateMgr->register_function('plugin_url', array(&$this, 'smartyPluginUrl'));
				$this->import('app.classes.form.PLNStatusForm');
