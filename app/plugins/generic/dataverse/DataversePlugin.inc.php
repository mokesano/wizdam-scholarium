...
require('core/library/swordappv2/swordappclient.php');
...
class DataversePlugin extends GenericPlugin {
	...
	function register($category, $path) {
		...
		if ($success && $this->getEnabled()) {
			...
			$this->import('app.classes.DataverseStudyDAO');
			...
			$this->import('app.classes.DataverseFileDAO');			
	...
	function manage($verb, $args, &$message, &$messageParams, $request = null) {
		...
		switch ($verb) {
			case 'connect':
				$this->import('app.classes.form.DataverseAuthForm');
				...
				return true;
			case 'select':
				$this->import('app.classes.form.DataverseSelectForm');
				...
				return true;
			case 'settings':
				$this->import('app.classes.form.SettingsForm');
	...
	function authorSubmitSuppFileFormExecute($hookName, $args) {
		...
		switch ($form->getData('publishData')) {
			...
			case 'dataverse':
				...
				if (!isset($dvFile)) {
					$this->import('app.classes.DataverseFile');
	...
	function createStudy($article) {
		...
		if ($depositReceipt->sac_status == DATAVERSE_PLUGIN_HTTP_STATUS_CREATED) {
			$this->import('app.classes.DataverseStudy');
